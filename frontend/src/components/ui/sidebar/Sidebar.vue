...
const computedCollapsible = computed(() => {
  if (isMobile) return props.collapsible
  return open ? 'none' : 'icon'
})
</script>
...
<template>
  ...
  <Sheet v-else-if="isMobile" :open="openMobile" v-bind="$attrs" @update:open="setOpenMobile">
    ...
    >
      <div class="flex h-full w-full flex-col bg-gray-300 opacity-80 dark:bg-neutral-900">
        <slot />
      </div>
    </SheetContent>
  </Sheet>
  ...
  <div
    ...
    :data-state="state"
    :data-collapsible="state === 'collapsed' ? computedCollapsible : ''"
  ...
  >
    ...
    <div
      :class="
        cn(
          'w-100 relative h-svh bg-transparent transition-[width] duration-200 ease-linear',
    ...
    <div
      :class="
        cn(
          ...
          side === 'left'
            ? 'left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] md:group-data-[collapsible=offcanvas]:left-0'
